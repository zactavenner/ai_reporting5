...
AS $function$
  ...
  funded_stats AS (
    ...
    -- missing or zero funded amount; commitment dollars stay a separate column.
    SELECT
      f.client_id,
      COUNT(*) FILTER (WHERE COALESCE(f.funded_amount, 0) > 0) AS funded_count,
      COALESCE(SUM(f.funded_amount) FILTER (WHERE COALESCE(f.funded_amount, 0) > 0), 0) AS funded_dollars,
      COALESCE(SUM(f.commitment_amount), 0) AS commitment_dollars,
      COALESCE(AVG(f.time_to_fund_days) FILTER (WHERE f.time_to_fund_days IS NOT NULL AND COALESCE(f.funded_amount, 0) > 0), 0) AS avg_time_to_fund,
      COALESCE(AVG(f.calls_to_fund) FILTER (WHERE COALESCE(f.funded_amount, 0) > 0), 0) AS avg_calls_to_fund
