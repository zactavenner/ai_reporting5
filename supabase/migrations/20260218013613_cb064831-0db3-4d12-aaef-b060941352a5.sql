...
-- RPC function to get per-client aggregated metrics from source tables
-- This bypasses the 1000-row limit by aggregating in the database
...
AS $$
  ...
  call_stats AS (
    ...
    WHERE (p_start_date IS NULL OR c.booked_at >= p_start_date)
      AND (p_end_date IS NULL OR c.booked_at <= p_end_date)
  ...
  funded_stats AS (
    ...
    WHERE (p_start_date IS NULL OR f.funded_at >= p_start_date)
      AND (p_end_date IS NULL OR f.funded_at <= p_end_date)
