...
// Sidebar navigation structure — Apple-style grouped
const NAV_SECTIONS = [
  {
    title: '',
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'ad-variations', label: 'Variations', icon: Wand2 },
...
export function CreativesTab() {
  ...
  return (
    <div className="flex gap-0 -mx-2 -mt-2">
      ...
      <div className={`flex-shrink-0 transition-all duration-500 ease-out ${sidebarCollapsed ? 'w-16' : 'w-56'}`}>
              ...
              {NAV_SECTIONS.map((section, sectionIdx) => (
                <div key={sectionIdx}>
                  {section.title && !sidebarCollapsed && (
                    <p className="text-[10px] font-semibold text-muted-foreground/50 uppercase tracking-[0.08em] px-3 mb-2">
                      {section.title}
                    </p>
                  )}
                  <div className="space-y-px">
                    {section.items.map(item => {
                      ...
                      return (
                        ...
                        >
                          <Icon className={`h-[15px] w-[15px] flex-shrink-0 ${isActive ? 'text-primary' : 'opacity-70'}`} />
                          {!sidebarCollapsed && (
                            <>
                              <span className="truncate">{item.label}</span>
                              {'isNew' in item && item.isNew && (
                                <span className="ml-auto inline-flex items-center text-[9px] font-semibold px-1.5 py-0 h-[15px] rounded-full bg-violet-500/15 text-violet-500">
                                  NEW
                                </span>
                              )}
                              {'showBadge' in item && item.showBadge && pendingCount > 0 && (
                                <span className="ml-auto inline-flex items-center justify-center text-[10px] font-semibold min-w-[20px] h-5 px-1.5 rounded-full bg-amber-500 text-white">
                                  {pendingCount}
                                </span>
                              )}
                            </>
      ...
      {/* Main Content Area */}
      <div className="flex-1 min-w-0 border-l border-border/50 pl-8 pr-2">
        {renderContent()}
      </div>
    </div>
  );
